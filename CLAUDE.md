...
- **Remote chunk URLs**: each remote sets `output.publicPath` to its absolute origin (`http://localhost:3101/` / `:3102/`) and sends `Access-Control-Allow-Origin: *`; otherwise the host would try to fetch remote chunks from its own origin.
